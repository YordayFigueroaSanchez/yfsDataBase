import datetime
import os
from classes import Batter
from classes import Pitcher
...
def extract_scorebox_meta(page,baseballGame):
    dataTable = []
    div = page.find('div', class_='scorebox_meta')
    div_s = div.find_all('div')
    for index,div_current in enumerate(div_s):
        tmpTextCurrent = div_current.text
        if index == 0:
            temDate = dateTo_YYYYMMDD(tmpTextCurrent)
            baseballGame.date = temDate
            dataTable.append(temDate)
            # print('Date:'  + temDate)
        if 'Start Time: ' in tmpTextCurrent:
            tempStartTime = extractSubStr(tmpTextCurrent,'Start Time: ')
            baseballGame.start_time = tempStartTime
        ...
        if 'Attendance: ' in tmpTextCurrent:
            ...
            baseballGame.attendance = tempAttendance
        ...
        if 'Game Duration: ' in tmpTextCurrent:
            ...
            baseballGame.game_duration = tempGameDuration
...
def extract_batting(page,baseballGame,status):
    dataTable = []
    if status == 'home':
        nameTeam = baseballGame.home_team.name
    else:
        nameTeam = baseballGame.away_team.name
    # div_HoustonAstrosbatting
    div_batting_team = 'div_' + nameTeam.replace(" ", "").replace(".", "") + 'batting'
    ...
    for index,div_tbody_tr in enumerate(div_batting_team_data_trs):
        ...
        if div_tbody_tr_th_a == None:
            ...
            batter = Batter()
            batter.name     = tmpName
            batter.id       = lastStrInUrlBeforeDot(tmpLink)
            # more data
            for indexTd,div_tbody_tr_td in enumerate(div_tbody_tr.find_all('td')):
                ...
                #  8 OBP	
                if indexTd == 8:
                    batter.OBP = tempValue
                #  9 SLG	
                if indexTd == 9:
                    batter.SLG = tempValue
                # 10 OPS	
                if indexTd == 10:
                    batter.OPS = tempValue
                # 11 Pit	
                if indexTd == 11:
                    batter.Pit = tempValue
                # 12 Str	
                if indexTd == 12:
                    batter.Str = tempValue
                # 13 WPA	
                if indexTd == 13:
                    batter.WPA = tempValue
                # 14 aLI	
                if indexTd == 14:
                    batter.aLI = tempValue
                # 15 WPA+	
                if indexTd == 15:
                    batter.WPA_plus = tempValue
                # 16 WPA-	
                if indexTd == 16:
                    batter.WPA_minus = tempValue
                # 17 cWPA	
                if indexTd == 17:
                    batter.cWPA = tempValue
                # 18 acLI	
                if indexTd == 18:
                    batter.acLI = tempValue
                # 19 RE24	
                if indexTd == 19:
                    batter.RE24 = tempValue
                # 20 PO	
                if indexTd == 20:
                    batter.PO = tempValue
                # 21 A	
                ...
                # 22 Details
                if indexTd == 22:
                    batter.Details = tempValue
                    extract_Details(batter, tempValue)
            if status == 'home':
                baseballGame.home_team.add_batter(batter)
            else:
                baseballGame.away_team.add_batter(batter)
            dataTable.append(dataTableRow)
